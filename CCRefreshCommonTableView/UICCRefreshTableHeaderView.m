...
- (void)setState:(kUICC_EGOPullRefreshState)aState {
	...
	switch (aState) {
		case kUICC_EGOPullRefreshPulling: { // {{{
			...
			[self.textLabel setText:NSLocalizedString(@"LCFreeMeYouGetMore",@"")]; // Release to refresh...
		...
		case kUICC_EGOPullRefreshNormal: { // {{{
            ...
            [self.textLabel setText:NSLocalizedString(@"LCPullDownToRefresh",@"")];// Pull down to refresh...
